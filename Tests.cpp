...
//! Проверка программы тестами, которые выводятся флагом -t
//! Вводимые значения из текстового файла
//! @return Результаты тестов
//! @retval true если все тесты прошли проверку успешно
//! @retval false если хотя бы 1 провалил проверку
...
bool TestSolve()
{
    ...
    if (Examples == 0)
    {
        printf("Not detected file");
        return false;
    }
    struct calc test[6] = {};
    for (int i=0; i<6; i++)
    {
        int w = fscanf(Examples, "%lg %lg %lg %lg %lg %d",&(test[i].a), &(test[i].b),
    ...
    for(int i=0; i<6; i++)
    {
        ...
        if (!(Compare(test[i].count, count)
           && Compare(test[i].x1, x1)
           && Compare(test[i].x2, x2)))
        {
